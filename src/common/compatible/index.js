...
let clientClassList = [WeChat,Android,Ios,Alipay];  //需要测试的客户端类型数组
...
Object.defineProperty(window.shareInst, 'client', {
  ...
  set: function (newValue) {
    if (newValue && newValue !== this._client) {
      //派发 clientChange 事件
      let clientChange = new Event("clientChange", {"bubbles": true});
      clientChange.value = newValue;
      clientChange.oldValue = this._client;

      this._client = newValue;

      window.dispatchEvent(clientChange);
    }
  }
});
