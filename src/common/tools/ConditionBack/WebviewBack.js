...
export class WebviewBack extends ConditionBack{
   ...
   */
  constructor({targetTypeID,backTypeID,condition,action,...superParams} = {}){
    super(superParams);
  ...
  get targetWebviewID(){
    ...
    let targetID = this.targetTypeID ;
    if (Object.values(specialWebviewTypeID).includes(targetID)){
      targetID = this.targetWebviewObject && this.targetWebviewObject.id ;
    }
...
//返回模式对应的处理器：开始
  ...
  back(locat){
    ...
    if (isCurrentWebview(this.targetWebviewID)) {
